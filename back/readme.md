...
- 23.11.16 CKH
  ...
  - 꽃 생성시 정원에 가입 승인 대기 중인 사용자에게는 알림이 가지 않도록 수정

- 23.11.22 CKH
  - 좋아요 랭킹 목록 조회 수정
    - 기존 로직은 비효율적이고 10개가 넘어가면 따로 좋아요 순으로 paging을 해서 요구사항 미충족
    - Object 배열로 결과를 받고 직접 매핑해주는 식으로 수정
    - 후에 MyBatis를 사용하면 좋을 것 같음